            ...
            switch (e.Action & MotionEventActions.Mask) {
                ...
                case MotionEventActions.PointerDown:
                    ...
                    if (activeTouches.Count < MAX_TOUCH_COUNT) {
                        ...
                        for(int i = 0; i< UIRenderer.Current.Count; i++) {
                            ...
                            if (item.Collides(touch.RelativePosition) && item.HandleTouch(UIActionType.Begin, touch)) {
                                break;
                            }
                        }
                    }
                ...
                case MotionEventActions.PointerUp:
                    ...
                    if (touchIndex != -1) {
                        for (int i = 0; i < UIRenderer.Current.Count; i++) {
                            UIItem item = UIRenderer.Current[i];
                            if (item.Collides(activeTouches[touchIndex].RelativePosition) && item.HandleTouch(UIActionType.End, activeTouches[touchIndex])) {
                                break;
                            }
                        }
                        ...
                        if (activeTouches[i].Position - activeTouchPosition != Vector2.Zero) {
                            ...
                            for (int j = 0; j < UIRenderer.Current.Count; j++) {
                                ...
                                if (current && !last) {
                                    ...
                                    if (item.HandleTouch(UIActionType.Enter, activeTouches[i])) {
                                        for(int k = j+1; k < UIRenderer.Current.Count; k++) {
                                            ...
                                            if (kitem.Collides(activeTouches[i].RelativePosition)) {
                                                kitem.HandleTouch(UIActionType.Leave, activeTouches[i]);
                                            }
                                        }
                                    }
                                } else if (!current && last) {
                                    // collides with the last position, but not with the current -> touch moved out of UIelement
                                    if(item.HandleTouch(UIActionType.Leave, activeTouches[i])) {
                                        for (int k = j + 1; k < UIRenderer.Current.Count; k++) {
                                            UIItem kitem = UIRenderer.Current[k];
                                            if (kitem.Collides(activeTouchRelativePosition)) {
                                                kitem.HandleTouch(UIActionType.Enter, activeTouches[i]);
                                                break;
                                            }
                                        }
                                        break;
                                    ...
                                    if (item.HandleTouch(UIActionType.Move, activeTouches[i])) break;
                                }
                            }
